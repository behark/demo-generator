export default function Hero() {
  return (
    <section
      id="home"
      className="relative min-h-[100svh] flex items-center justify-center overflow-hidden bg-[#0A1628]"
    >
      <div
        className="absolute inset-0 kenburns bg-cover bg-center"
        style={{ backgroundImage: "url('https://images.unsplash.com/photo-1492144534655-ae79c964c9d7?auto=format&fit=crop&w=2400&q=80')" }}
        aria-hidden
      />
      <div
        ...
        style={{
          background:
            "linear-gradient(180deg, rgba(0,0,0,0.35) 0%, rgba(0,0,0,0.55) 45%, rgba(10,22,40,0.94) 100%)",
        }}
        aria-hidden
      />

      <div className="relative z-10 max-w-5xl mx-auto px-6 text-center text-white">
        <div className="eyebrow text-[#D4B07E] fade-up fade-up-1">
          Autosalloni · Vetura · Garanci
        </div>
        ...
        <h1 className="font-[family-name:var(--font-playfair)] font-semibold tracking-tight mt-6 mb-5 text-[clamp(3rem,7.5vw,6.25rem)] leading-[0.95] fade-up fade-up-2">
          Vetura të besuara,
          <span className="block italic font-normal text-[#D4B07E] -mt-1">
            të zgjedhura për ju.
          </span>
        </h1>

        <p className="max-w-2xl mx-auto text-lg sm:text-xl text-white/75 font-light leading-relaxed fade-up fade-up-3">
          Importim direkt, kontroll teknik, garanci me shkrim. Blejeni veturën e radhës pa stres dhe pa surpriza.
        </p>

        <div className="flex items-center justify-center gap-6 mt-10 fade-up fade-up-3">
          <div className="flex items-center gap-3 text-white/70 text-sm">
            <span className="w-8 h-px bg-[#D4B07E]" />
            Importim nga Gjermania dhe Zvicra
            <span className="w-8 h-px bg-[#D4B07E]" />
          </div>
        </div>
        ...
        <div className="flex flex-col sm:flex-row items-center justify-center gap-3 mt-10 fade-up fade-up-4">
          <a
            href="#kontakt"
            className="group inline-flex items-center gap-2 bg-[#D4B07E] text-[#0A1628] font-medium px-7 py-3.5 rounded-full text-[0.95rem] tracking-wide transition-all hover:bg-white w-full sm:w-auto justify-center"
          >
            Na Kontaktoni
